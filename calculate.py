...
for benchmark in listdir(pathname):
    ...
    for pfile in listdir(pathname + "/" + benchmark + "/"):
	    ...
	    if not d[benchmark].has_key(arch):
            ...
            if not count == 0:
                d[benchmark][arch][method][metric][agap][branch][relax]['avg'] = { 'num':round(sumhd/count,2),'maxnum': maxnum, 'data': intd.values()}
            ...
            if not count == 0:
             	d[benchmark][arch][method][metric][agap][branch][relax]['bravg'] = {'num': round(sumhd/count,4), 'maxnum': maxnum, 'data': intd.values()}
