...
#for llfile in ../unison/src/unison/test/fast/Mips/speed/*.ll
    do
        clang-3.8 $CFLAGS -O2 -target mipsel -mllvm -debug-pass=Arguments -S -emit-llvm $cfile
        rm /tmp/unison-*
        fname1=${cfile##*/}
        fname=${fname1%.*}
        function=fname
        ...
        timeout 1m llc $fname.ll $opt -march=mips -mcpu=mips32 -unison -unison-no-clean -unison-only-functions="$function"
